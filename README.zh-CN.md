...
把 ChatGPT 网页版变成 OMP 编码任务的"规划与审查大脑"，而执行权完全保留在
OMP 手里。你的工作区永远不会被上传——ChatGPT 通过一条安全的、OAuth 保护的
...
唯一可能需要你动手的步骤：登录 ChatGPT（如果要用固定域名，再登录一次 Cloudflare）。**新工作区**还会请你在 ChatGPT 里建一次项目（合集）：名字用工作区目录名，记忆选「仅限项目记忆」。侧栏如果没有「项目」，把鼠标放在「聊天」上，点右边三个点，选「按项目整理」。之后对话都从合集页开，不用回首页。已经在用的工作区默认还是原来的一条长对话，除非你说要改成 Project。
