...
description: >
  Use ChatGPT (web) as the planning and review brain for OMP coding sessions,
  while OMP keeps full execution ownership. Use when the user says
  "使用 OMP with ChatGPT ..." / "用 ChatGPT 规划" / "C2C",
...
You (OMP) own execution: editing, shell, git, tests, recovery.
...
4. If something fails, run `c2c doctor` and repair silently. Only involve the user
   ...
   once.
5. ALWAYS use the **dedicated C2C browser** for every ChatGPT step. Follow
   **Dedicated browser (ChatGPT)** below. NEVER Computer Use (no
   screenshot-click). NEVER relay into, launch, or control the user's daily
   browser (their Chrome / Safari / Edge profile), and never use
   ...
   - The ONLY exception: the user explicitly says the Cloudflare login must use
     ...
     their browser; everything else stays in the dedicated browser.
   - If the user asks to run ChatGPT in their own browser, refuse politely and
     explain: "OMP 需要持续调用 ChatGPT 和配置连接，这会频繁操作页面，可能影响
     你浏览器的正常使用。ChatGPT 只能跑在专用浏览器里。" Only if the user replies
     with an explicit "我愿意承担影响" may you proceed in their browser; otherwise
     keep ChatGPT in the dedicated browser, every time they ask.
6. Conversation reuse depends on `c2c session --json` → `conversation.mode`
   ...
   - **project** (new workspaces, or an existing workspace that opted in):
     ONE ChatGPT Project (collection) per workspace. The same OMP session
     reuses the ChatGPT chat URL saved in THIS session. A new OMP session
     opens a new chat from the Project collection page — never `goto`
     `https://chatgpt.com/` to create it, and never reuse another OMP
...
7. One workspace = one active C2C task, owned by the main OMP session.
   A task starts only with `/c2c-enable "<goal>"` and ends with
   `/c2c-finish` or `/c2c-cancel`. A new goal after DONE needs a new
   `/c2c-enable`. Subagent sessions can read status but can never enable
   or take over a task — the extension blocks it; do not work around that.
   While the task waits for review (`waiting=GPT_REVIEW`), your modifying
   tools are gated and `session_stop` will block ending the session. That
   is intentional: review completes before new edits.
8. ChatGPT pages: only the URLs in **Dedicated browser (ChatGPT)**. Never start
...
9. **Doctor gate.** After `c2c doctor --json`, do not `goto` ChatGPT and do not
   ...
   connector. ChatGPT-side-only errors are not permission to churn the
   public address.
...
## Dedicated browser (ChatGPT)

ChatGPT runs in a dedicated automation browser, never the user's daily one.
The backend is selected explicitly — never an unqualified `browser.open`.

1. **Resolve and verify the backend first** (once per OMP session):
   `c2c browser status --json` → `{ backend, cdpUrl, profileDir, verification }`.
   - `verification.ok` is true → use it:
     - `backend: "omp"` → `browser.open({ name: "c2c-chatgpt", url, persist: true })`
       (OMP-managed dedicated Chromium).
     - `backend: "cdp"` → `browser.open({ name: "c2c-chatgpt", url, persist: true, app: { cdp_url: "<cdpUrl>" } })`
       (e.g. WSL2: dedicated Windows Edge with an isolated profile).
   - `verification.ok` is false → give the user exactly
     `verification.action` (one action), wait, then verify again. Do not
     improvise another browser.
   - Never `app.relay`. Never a profile directory that is a daily-browser
     location — `c2c browser set` rejects those; do not work around it.
2. **One tab, kept alive.** Open the ChatGPT tab once with `persist: true`
   and the fixed name `c2c-chatgpt`. After that, only `tab.goto(...)` to
   switch URLs. If the tab already exists (`browser.tab("c2c-chatgpt")`),
   claim it — never open a second ChatGPT tab. Do not `goto` the URL you
   are already on. Never close this tab: finished, waiting for the user, or
   timed out — leave it open (standby). `persist: true` keeps it across
   turn boundaries.
3. **Login persistence comes from the dedicated profile.** The same profile
   directory is reused every time, so the ChatGPT login survives browser
   restarts. Never clear cookies/storage, never switch profileDir, never
   "fix" a logged-out browser by switching profiles. If the tab shows a
   login wall, that is a user action (see below), not a profile problem.
4. **URLs only** (same tab, `goto` — never hunt menus):
   ...
   - Saved C2C chat: `conversation.chatUrl` / `session.url` (long-chat, or
     the chat already bound in THIS OMP session)
...
5. **Do not wait for 8 tools** on the settings page. "Connected" / authorize
   ...
   `workspace_info`.
6. **Batch.** Fill a known form in one `tab.run(...)` script when you can.
...
7. **One conversation, Chat mode.** The first ChatGPT chat is the C2C
   ...
   the session URL. If validation fails, keep the old saved URL. Do not open
   a throwaway verify chat and later another C2C chat.

   A collection or chat page that shows only `Retry` / `重试` is a navigation
   error, not generation and not a pairing failure. Reuse the same tab.
   Try Retry once. If it stays Retry-only, `goto` the last working chat URL
   from this session (or `session.url` if that is the only saved chat), then
...
8. **Wait for a ChatGPT reply (do not hold one long browser wait).** After you
   send INIT, EXECUTED, boot, or the workspace_info check: keep the tab and
   stay in this same task. Do not `waitFor` 5 minutes and do not
   screenshot-poll. Every 20–30 seconds, one cheap DOM check:
...
9. **Uncertain send = stop and re-observe.** Browser sending is not
   transactional. If you cannot confirm a control message landed (page
   error, navigation, unclear DOM), do NOT resend and do NOT advance the
   checkpoint. Re-observe the original conversation until you can confirm
   whether the message is there; only then continue. If you cannot confirm,
   pause and tell the user the ONE thing you need.

## Locations

- The omp-with-chatgpt checkout lives at: `<ACTUAL_CHECKOUT_PATH>`
...
- If the checkout has no `node_modules`, first run `corepack pnpm install` inside it.
- The OMP extension and this Skill load from the same checkout via
  `omp plugin link <absolute checkout>`; there is nothing to copy.
...
- Do not add `-w` to machine-wide commands: `update-check`, `prefs`,
  `tunnel login`, `browser`. They still accept and ignore `-w`, so a
  leftover flag must not fail the command.

## Daily update check

At the START of every workflow below (before anything else), run
`c2c update-check --json` (cheap / cached; never mention it unless an update
exists):

- `{ "updateAvailable": false }` → continue silently. Never mention the check.
- `{ "updateAvailable": true }` → tell the user one line:
  "检测到 OMP with ChatGPT 有新版本，我先更新一下（约 1 分钟），随后继续你的任务。"
  Then run the update workflow below, and CONTINUE the original task afterwards.
  If a C2C task is currently active (`/c2c-status` shows one), tell the user the
  update waits until the task finishes, and continue the task instead.

## Workflow: update（"更新 OMP with ChatGPT"，or triggered by the daily check）
...
4. Tell the user "✓ 已更新到最新版本" — then resume whatever task triggered this.
   (The updated Skill takes effect from the next OMP session; that's expected.)
...
## Workflow: first-time setup（"使用 OMP with ChatGPT 完成首次配置"）
...
2. If the c2c repo has no `node_modules`, run `pnpm install` in it.
3. Run **Connection choice**, then `c2c setup -w <workspace> --json`.
   → returns `{ mcpUrl, pairingCode, workspaceName, connectorName, ... }`.
   `connectorName` is this workspace's plugin title (first workspace stays
   `OMP with ChatGPT`; additional workspaces get `OMP with ChatGPT · <name>`).
...
4. `c2c prefs --json` (this machine, not this workspace).
   ...
   - `setupMode: "manual"`: skip step 6's automatic ChatGPT settings. Go to
     **Guided manual ChatGPT setup** (chosen). Opening line:
     `接下来用手动教学配置。一次只需要做一个操作。`
     Do not say 自动配置没有成功.
   - `setupMode: "auto"`: continue with step 6. Keep the two-failure fallback.
5. Resolve and verify the browser backend: `c2c browser status --json`
   (see **Dedicated browser**). Do not continue until verification is green.
6. Open ChatGPT on the ONE dedicated tab (see **Dedicated browser**).
   `persist: true` immediately. Same tab, `goto` only:
   ...
   - 还没有 / 刚删掉: `https://chatgpt.com/plugins#settings/Connectors?create-connector=true&redirectAfter=%2Fplugins`
     Operate ONLY on `connectorName` from step 3:
      ...
      - Description: `Securely connect ChatGPT to the current OMP workspace for planning and review.`
     ...
     tools on this page.
7. Same tab: open the first C2C chat per **Conversation management**
   (Project collection for a new workspace; `https://chatgpt.com/` only
   in long-chat). Confirm Chat mode per **Dedicated browser** §7 (if it is Work,
   ...
   Confirm the reply matches `workspaceName` (wait per **Dedicated browser** §8).
   Only then save the chat URL with `c2c session set` (see Conversation
   management). If the name does not match, do not save.
8. Report to the user exactly in this shape (no internals):
...
silently fall back to OMP-only execution and do not permanently disable C2C.
...
- **Find it**: if `conversation.reuseSavedChat` and `conversation.chatUrl`,
  `goto` that URL and continue there.
...
- **Update it**: after each EXECUTED/DONE,
  ...
  The task checkpoint itself (protocol state, waiting, next step) lives in the
  extension — drive it with `/c2c-checkpoint` (see the coding workflow); do not
  duplicate it here. Do not put logs or diffs in any field.
- **Switch it** ONLY when (a) the user asks for a new chat, (b) the current
  chat visibly lags, or (c) this conversation is Work. Then:
  1. Same tab: `goto` `https://chatgpt.com/`, confirm Chat mode
     (**Dedicated browser** §7), then send the boot prompt.
...
- Saved chat 404s: treat as a switch. Reconstruct HANDOFF from
  `/c2c-status` (goal, progress, issues, next step). If there is no
  checkpoint, use `task` / `iteration` / `state` and `execution_summary`
...
1. Same OMP session (this session still has context) → same ChatGPT
   chat URL. `goto` that URL directly. Do not open the collection first.
2. Same workspace, a **new** OMP session → new ChatGPT chat from the
   collection page (`conversation.projectUrl`). Ignore `session.url` unless
   you already saved it earlier in THIS OMP session. Exception: resuming an
   unfinished task follows the recovery rules below — an unfinished task
   continues its original chat when it still exists.
3. Different workspace → different Project and different connector.

**Open a chat in this OMP session**

- If you already saved a ChatGPT chat URL earlier in THIS OMP session:
...
- Else if `conversation.projectReady`: `goto` `conversation.projectUrl`.
  ...
  Confirm Chat mode (**Dedicated browser** §7). Boot prompt, then workspace_info
  ...
  If this OMP session is continuing a previous C2C task, send HANDOFF right
...
**Saved chat 404s** (this session): `goto` the collection, open a new chat
there, boot + HANDOFF from `/c2c-status` (no logs) + workspace_info,
...
(Computer Use is forbidden; the dedicated tab must not hunt that menu).
...
2. Wait for「好了」/ the collection page. Same tab: read the address bar.
...
4. Still on the collection page, create the first chat with the on-page
   composer, then boot + workspace_info as in setup step 7. Save the chat URL.
...
## Workflow: coding task（"使用 OMP with ChatGPT 完成 XXX"）

Protocol states sent to ChatGPT: INIT → PLAN → EXECUTING → EXECUTED → REVIEW → (PLAN | DONE | BLOCKED).
Local checkpoint states (extension only, never a ChatGPT `STATE:` line):
...
All control messages start with `[C2C]`. Keep OMP→ChatGPT messages under 1 KB.
ChatGPT's replies are expected to be substantive (see step 4). Docs: `docs/protocol.md`.
...
0. `c2c tunnel status -w <workspace> --json`. If `needsChoice`, follow
   **Connection choice** first (existing installs: ask once, then remember).
   ...
   Verify the browser backend: `c2c browser status --json` must be green.
1. **Enable the task.** `/c2c-status` first:
   - No active task → `/c2c-enable "<user's goal>"`. It returns the task id
     (`c2c_…`) and makes THIS OMP session the owner. One workspace = one
     active task; if another session owns one, finish or `/c2c-takeover`
     after that session has exited — never run two tasks.
   - Active task exists → this is a continuation; skip enable and resume
     from the checkpoint below. Do not mint a second task id.
2. `c2c session -w <workspace> --json`. Open ChatGPT on the same dedicated
   tab per **Conversation management** for `conversation.mode`. long-chat:
   saved chat, or `https://chatgpt.com/` if none. project: this session's
   chat URL, or the collection page for a new chat, or **Bind Project** if
   `projectReady` is false. On a NEW conversation confirm Chat mode
   (**Dedicated browser** §7), then send the boot prompt from
   ...
   **Resume from the checkpoint before any INIT.** `/c2c-status` reports the
   task's checkpoint (`state`, `waiting`, `nextStep`, goal, issues). A
   browser/js timeout is not a lost task — claim the original tab; do not
   INIT, re-run, or resend EXECUTED just because a wait timed out.
   - `EXECUTED_SENT` + `waiting=GPT_REVIEW`: do not INIT, do not re-run,
   ...
   - `INIT` / `waiting=GPT_PLAN`: claim the tab and wait. Do not resend INIT.
   - `DONE`: summarize to the user if needed; `/c2c-finish`.
...
3. Send INIT with the user's goal (skip when the checkpoint says not to).
   TASK_ID is the id `/c2c-enable` returned:
...
INSTRUCTION:
Inspect the connected workspace through the OMP with ChatGPT MCP connector.
Produce a C2C PLAN message.
```

   Confirm the INIT message is visibly in that ChatGPT conversation (one cheap
   DOM check). If the page is Retry-only, recover per **Dedicated browser** §7
   first. Do not write the waiting checkpoint, and do not wait for PLAN, until
   that message is visible. On an uncertain send, follow **Dedicated browser**
   §9: re-observe, never blind-resend.
   Then: `/c2c-checkpoint "state=INIT waiting=GPT_PLAN next=wait for PLAN"`
4. Wait for ChatGPT's `STATE: PLAN` reply (**Dedicated browser** §8 — short DOM
...
6. Record the execution so ChatGPT can read it via MCP. Metadata always:
   ...
   and iteration must match `/c2c-status`.
...
2. Optionally remove the connector on the same dedicated tab via
   `https://chatgpt.com/plugins`. Only touch this workspace's `connectorName`.
3. Tell the user: "已断开 ChatGPT 对该项目的访问。"

## Workflow: reconnect after address reclaim（全关掉以后地址失效）

This is the normal case when the user quit OMP / the terminal / the machine:
...
2. Verify the browser backend (`c2c browser status --json`), then use the
   same one dedicated tab as setup (`persist: true`). Settings URLs only
...
4. `c2c doctor --json` again. Same tab: only after the Doctor gate is green,
   reopen the chat this OMP session was already using (`session.url` /
   the URL you saved earlier in THIS session). Do not rewrite Project
   instructions — they store the connector **name**, which did not change.
   In that same chat, send the workspace_info check from setup step 7
   ...
   - If workspace_info fails, times out, or cannot read the name: do **not**
     ...
     Project, boot + HANDOFF from `/c2c-status` (no logs) +
...
| Browser backend not verified | `c2c browser status --json` → give the user exactly `verification.action` (one action), then verify again. Never fall back to the user's daily browser. |
| ChatGPT tab logged out | Login persists in the dedicated profile; a login wall means the session expired. Give the user ONE action ("请在专用浏览器里登录 ChatGPT，完成后告诉我'好了'"), keep the same tab and profile. Never switch profiles or clear storage. |
| Collection page shows only Retry | Same tab: Retry once, then open the last working chat and click its Project link. Do not write INIT/EXECUTED waiting checkpoints until the message is visible. |
