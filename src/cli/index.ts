...
program
  .name("c2c")
  .description(`${PRODUCT_NAME} — ChatGPT thinks. OMP works.`)
...
program
  ...
  .action(async (opts: { workspace?: string; fix: boolean; json: boolean }) => {
    ...
    // OMP keeps its state under its own state directory; no external sandbox config.
...
program
  ...
  .description("Record an OMP execution summary (used by the Skill)")
  .option("-w, --workspace <path>")
